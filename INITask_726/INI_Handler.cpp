...
void setPolizationDirection(double * inc_dir, double angle_, double& pol_theta, double& pol_phi){
        ...
        for(int i = 0; i < 3; i++){        
            pol_dir[i] = theta_xyz[i] * std::sin(angle)
                         - phi_xyz[i] * std::cos(angle);
        }


        pol_theta = std::acos(pol_dir[2]);
        pol_phi = std::acos(pol_dir[0] / std::sin(pol_theta));

        pol_theta = pol_theta * 180.0 / M_PI;
        pol_phi = pol_phi * 180.0 / M_PI;

    }

int INI_Handler::Incf_file(INIReader reader,const string& incf_flie){
    double inc_dir[2], pol_theta, pol_phi;
    ...
    angle = reader.GetReal("INCIDENT_WAVE", "PolarizationAngle", 0);
    ...
    //计算传播方向球坐标，并写入
    setPolizationDirection(inc_dir, angle, pol_theta, pol_phi);
    //分别写入极化方向球坐标
    odat_INCF<<std::setiosflags(std::ios::fixed) <<std::setprecision(1)<<"    "<<pol_theta<<"    "
             <<pol_phi<<"\t\t"<<'#'<<"  Spherical coordinates of polarization direction /° TheE0,PhiE0"<<"\n";
    odat_INCF<<std::setiosflags(std::ios::fixed) <<std::setprecision(1)<<"    "<<inc_dir[0]<<"    "
             <<inc_dir[1]<<"\t\t"<<'#'<<" Spherical coordinates of propagation direction /°\n";
...
int INI_Handler::Wfb_file(INIReader reader,const string& wfb_flie,std::map<string,int>& pre_, std::map<string,int>& type_,std::array<double,15>& wfb_model){
    ...
    odat_Wfb<<1.0<<"\t\t"<<'#'<<"  Integral weight of CFIE [1.0]=EFIE;  [0.0]=MFIE\n";
    ...
    //频点
    wfb_model[0] = 1.0;  wfb_model[1] = atof(v_wfb[0].c_str());  wfb_model[2] = atof(v_wfb[1].c_str());  wfb_model[3] = freq_num*1.0;  wfb_model[4] = (pre_[f_key])*1.0;
    wfb_model[5] = type_[f_key]*1.0;  wfb_model[6] = reader.GetReal("SOLVER", "Residual", 0);  wfb_model[7] = reader.GetReal("SOLVER", "Max_Iter", 0);
    wfb_model[8] = 40; wfb_model[9] = re_cu*1.0; wfb_model[10] = 1.0;
    wfb_model[11] = reader.GetReal("NEARFIELD", "NearFieldNumber", 0);  wfb_model[12] = 100000;  wfb_model[13] = 2;
    wfb_model[14] = 1.0;
    if(freq_num>1){
...
int INI_Handler::mid_elem(std::ifstream& infile, std::vector<std::string>& geo_v, const string& out_file){
    ...
    //写四面体各属性的elements
    for(std::size_t i=0;i<elem_4.size();i++){
    ...
    //写三角形各属性的elements
    for(std::size_t i=0;i<elem_2.size();i++){
        outfile_midelem<<"  "<<2;
        outfile_midelem<<"  "<<i+1;
        for(int j=2;j<11;j++)
    ...
    numbers_of_elements_2 = elem_2.size();
    for(std::size_t i=0;i<boudaries.size();i++){
        outfile_midelem<<"  "<<2;
...
//生成所有ini文件
int INI_Handler::ini_out(std::array<double,15>& wfb_model){
    // 创建文件夹
    ...
    std::vector<string> key_pre = {"diagonal","normalized_BF","no"};

    for(std::size_t i=0; i<key_pre.size();i++){
    ...
    _error = Wfb_file(reader,Wfb_path.c_str(),pre_,type_,wfb_model);
    if(_error!=0)
        return _error;
